...
$port = new Device::SerialPort( '/dev/ttyr0d' );
$port->baudrate(2400);
$port->databits(8);
$port->parity('none');
$port->stopbits(1);
...
while (1)
{
  my ( $count, $rx_msg ) = $port->read(1);

  unless( $count ) { next; }
  
  my $rx_msg_up = unpack "C", $rx_msg;

  unless ( $rx_msg_up == 0xFF ) 	{ print "Out of sync\n"; next; }
		
  my ( $count, $rx_msg ) = $port->read(6);
	...
	#print Dumper \@rx_msg; 
  #print( "Rx1: ".$count." bytes msg: $rx_msg_up\n" );
...
sub print_pelcod
{
	...
	my @rx_msg = split //, $rx_msg; 

	if (  ord($rx_msg[2]) == 0 ) { next; }
	
	my $id = unpack "C", $rx_msg[0];
	print "ID: $id\t";   

	my $cmd1 = unpack "B8", $rx_msg[1];
	print "CMD1: $cmd1\t";   

	my $cmd2 = unpack "B8", $rx_msg[2];
	print "CMD2: $cmd2\t";   
	
	my $data1 = unpack "B8", $rx_msg[3]; 
	print "Data1: $data1\t";   

	my $data2 = unpack "B8", $rx_msg[4]; 
	print "Data2: $data2\n";   
} 	
